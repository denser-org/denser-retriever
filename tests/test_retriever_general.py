...
class TestRetrieverGeneral:
    def setup_method(self):
        ...
        """
        self.retriever_denser = RetrieverGeneral(
            "unit_test_denser", "tests/config-denser.yaml"
        )
        self.retriever_titanic = RetrieverGeneral(
            "unit_test_titanic",
            "tests/config-titanic.yaml",
        )
        self.retriever_cpws = RetrieverGeneral(
            "unit_test_cpws", "tests/config-cpws.yaml"
        )

    def test_ingest(self):
        self.retriever_denser.ingest(
            "tests/test_data/denser_website_passages_top10.jsonl"
        )
    ...
    def test_retrieve_denser(self):
        topk = self.retriever_denser.settings.rerank.topk
        ...
        assert abs(passages[0]["score"] - 1.1621) < 0.01
