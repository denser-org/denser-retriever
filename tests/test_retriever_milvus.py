...
class TestRetrieverMilvus:
    def setup_method(self):
        ...
        """
        self.retriever_milvus_denser = RetrieverMilvus("unit_test_denser", "tests/config-denser.yaml")
        self.retriever_milvus_titanic = RetrieverMilvus("unit_test_titanic", "tests/config-titanic.yaml")
        self.retriever_milvus_cpws = RetrieverMilvus("unit_test_cpws", "tests/config-cpws.yaml")

    def test_ingest(self):
        self.retriever_milvus_denser.ingest("tests/test_data/denser_website_passages_top10.jsonl", 10)
        self.retriever_milvus_titanic.ingest("tests/test_data/titanic_top10.jsonl", 10)
        self.retriever_milvus_cpws.ingest("tests/test_data/cpws_passages_top10.jsonl", 10)

    def test_retrieve_denser(self):
        topk = self.retriever_milvus_denser.config["vector"]["topk"]
    ...
    def test_retrieve_titanic(self):
        topk = self.retriever_milvus_titanic.config["vector"]["topk"]
        query = "cumings"
        meta_data = {"Sex": "male"}
