...
class TestRetrieverElasticSearch:
    def setup_method(self):
        ...
        self.retriever_denser = RetrieverElasticSearch("unit_test_denser", "tests/config-denser.yaml")
        self.retriever_titanic = RetrieverElasticSearch("unit_test_titanic", "tests/config-titanic.yaml")
        self.retriever_cpws = RetrieverElasticSearch("unit_test_cpws", "tests/config-cpws.yaml")

    def test_ingest(self):
        self.retriever_denser.ingest("tests/test_data/denser_website_passages_top10.jsonl", 10)
    ...
    def test_retrieve_titanic(self):
        ...
        topk = self.retriever_titanic.config["keyword"]["topk"]
        assert len(passages) == topk
