...
def process_queries_all_methods(
    ...
    for i, query in enumerate(queries):
        ...
        for method_name in methods:
            ...
            results_method, _, token_metrics = method_func(
                query=query_str,
                k=top_k,
            ...
            if usage and token_metrics:
                token_stats[method_name]["vector_tokens"] += token_metrics.get("vector_tokens", 0)
                token_stats[method_name]["reranker_tokens"] += token_metrics.get("reranker_tokens", 0)
    ...
    if usage:
        for method_name in methods:
            ...
            if num_queries > 0:
                stats["avg_vector_tokens"] = stats["vector_tokens"] / num_queries
                stats["avg_reranker_tokens"] = stats["reranker_tokens"] / num_queries
...
def calculate_method_costs(token_stats: Dict[str, Dict[str, int]], method: str) -> Dict[str, float]:
    ...
    # Cost calculations
    es_cost = costs['es_query_cost'] * num_queries if method != "vector" else 0.0  # $0.001 per query
    vector_cost = (stats["vector_tokens"] * costs['vector_token_cost_per_million']) / 1_000_000  # $0.08 per million tokens
    reranker_cost = (stats["reranker_tokens"] * costs['reranker_token_cost_per_million']) / 1_000_000  # $0.05 per million tokens
...
def main():
    ...
    for method in token_stats.keys():
        ...
        print(f"Vector Tokens: {stats['vector_tokens']:,} "
              f"(avg: {stats.get('avg_vector_tokens', 0):,.1f} per query)")
        print(f"Reranker Tokens: {stats['reranker_tokens']:,} "
